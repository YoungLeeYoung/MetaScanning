...
`.trim(),
  ...
  ranking: {
    ...
    /**
     * 四个维度的权重（加起来不用等于 1，会各自乘上去）。
     *
     * 这是全项目最值得动手调的一项，因为它直接对应一个价值判断：
     *   兴趣匹配调高 → 更愿意看还没人发现的新项目，但噪音也更多
     *   早期动量调高 → 优先看当天已经有人验证过的，但会漏掉冷启动的好东西
     *
     * 默认偏兴趣一点（0.45 / 0.20），适合「自己先看到」的用法。
     * 如果你觉得榜单被一堆 1 star 的关键词堆砌项目占满，就把 momentum 提到 0.35 左右。
     */
    weights: {
      interest: 0.45, // 和你配置的方向有多契合
      quality: 0.25, // 文档、license、demo 站点等完成度信号
      momentum: 0.2, // 当天拿到的 star / fork
      author: 0.1, // 这个作者以前收录过的项目表现如何
    },
  },
